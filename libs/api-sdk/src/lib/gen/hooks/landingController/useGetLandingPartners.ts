...
type GetLandingPartnersClient = typeof client<
  GetLandingPartnersQueryResponse,
  never,
  never
>;
...
export function getLandingPartnersQueryOptions<
  TData = GetLandingPartners['response']
>(
  options: GetLandingPartners['client']['parameters'] = {}
): SWRConfiguration<TData, GetLandingPartners['error']> {
  return {
    fetcher: async () => {
 ...
 */
export function useGetLandingPartners<
  TData = GetLandingPartners['response']
>(options?: {
  ...
  const query = useSWR<TData, GetLandingPartners['error'], typeof url | null>(
