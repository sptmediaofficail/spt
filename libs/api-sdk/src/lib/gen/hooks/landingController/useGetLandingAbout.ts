...
type GetLandingAboutClient = typeof client<
  GetLandingAboutQueryResponse,
  never,
  never
>;
...
export function getLandingAboutQueryOptions<
  TData = GetLandingAbout['response']
>(
  options: GetLandingAbout['client']['parameters'] = {}
): SWRConfiguration<TData, GetLandingAbout['error']> {
  return {
    fetcher: async () => {
 ...
 * @link /landing/about
 */
export function useGetLandingAbout<
  TData = GetLandingAbout['response']
>(options?: {
  ...
  const query = useSWR<TData, GetLandingAbout['error'], typeof url | null>(
